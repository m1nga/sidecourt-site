...
SIDECOURT_ID = re.compile(r'^SC[23456789ABCDEFGHJKMNPQRSTUVWXYZ]{5}$')
...
def inject_head(application, title, description, canonical, og_type='website', image='', author='', review_site=False):
    ...
    head = re.sub(r'<meta\s+name="author"\s+content="[^"]*"\s*/?>', '', head, flags=re.IGNORECASE)
    if author:
        head += '<meta name="author" content="' + esc(author) + '">\n'
    if review_site:
        head = re.sub(r'<meta\s+name="robots"\s+content="[^"]*"\s*/?>', '', head, flags=re.IGNORECASE)
...
def public_pages(works, site_url=SITE):
    ...
    for work in works if isinstance(works, list) else []:
        ...
        if isinstance(work_id, str) and UUID.match(work_id):
            ...
            description = summary(collapse(data.get('cardLine')) or collapse(data.get('card_line')) or data.get('description')) or 'A public post on SideCourt.'
            cover = data.get('cover') if isinstance(data.get('cover'), str) else ''
            short_id = collapse(work.get('sidecourt_id')).upper()
            short_id = short_id if SIDECOURT_ID.fullmatch(short_id) else ''
            canonical = site + '/work/' + (short_id or work_id + '/')
            for address in dict.fromkeys([work_id, short_id or work_id]):
                pages.append(('work/' + address, name + ' · SideCourt', description, canonical, 'article', cover))
...
def publish_public_pages(out, application, works, config=None):
    ...
    authors = {}
    for work in works if isinstance(works, list) else []:
        if isinstance(work, dict) and isinstance(work.get('data'), dict):
            for address in [work.get('id'), collapse(work.get('sidecourt_id')).upper()]:
                if isinstance(address, str):
                    authors['work/' + address] = collapse(work['data'].get('author'))
    for folder, title, description, canonical, og_type, image in pages:
        target = pathlib.Path(out) / folder
        target.mkdir(parents=True, exist_ok=True)
        (target / 'index.html').write_text(inject_head(application, title, description, canonical, og_type, image, authors.get(folder, ''), bool(config.get('reviewSite'))))
    return pages
